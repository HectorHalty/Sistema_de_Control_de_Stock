...
import { ChevronDown, ChevronUp, Check, Download, Calendar, ClipboardList } from 'lucide-react';
import type { ConsumptionLog } from './store';
import { getUnitLabel } from './store';
import { downloadBlobFile } from './download';
import { buildConsumptionReportXlsx } from './xlsxExport';
...
export function ConsumptionPage() {
  const { products, warehouses, setProducts, consumptionLogs, setConsumptionLogs, addAudit } = useAppContext();
        ...
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-[#717182]">{lastLog.date} · {lastLog.dateType === 'after' ? 'After / Especial' : 'Regular'} · {lastLog.entries.length} productos</p>
            </div>
          </div>

          {/* Report table */}
          <div className="rounded-lg border border-[rgba(0,0,0,0.08)] overflow-hidden mb-4">
            ...
            {/* Mobile view */}
            <div className="block sm:hidden divide-y divide-[rgba(0,0,0,0.06)]">
              {lastLog.entries.map((e, i) => (
                <div key={i} className="px-4 py-3 bg-card">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${e.consumed > 0 ? 'bg-orange-100 text-orange-700' : e.consumed < 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`} style={{ fontWeight: 600 }}>
                      {e.consumed > 0 ? '-' : e.consumed < 0 ? '+' : ''}{Math.abs(e.consumed)} {getUnitLabel(e.unit, true)}
                    </span>
                  </div>
                  <p className="text-xs text-[#717182]">{e.warehouseName}</p>
                  <div className="flex gap-4 mt-1 text-xs text-[#717182]">
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {lastLog.entries.map((e, i) => (
                    <tr key={i} className="border-t border-[rgba(0,0,0,0.05)]">
                      ...
                      <td className="px-4 py-2.5 text-sm text-[#717182]">{e.warehouseName}</td>
                      <td className="px-4 py-2.5 text-sm text-right text-[#717182]">{e.previousStock}</td>
                      <td className="px-4 py-2.5 text-sm text-right" style={{ fontWeight: 500 }}>{e.newStock}</td>
                      <td className={`px-4 py-2.5 text-sm text-right ${e.consumed > 0 ? 'text-orange-600' : e.consumed < 0 ? 'text-blue-600' : 'text-[#717182]'}`} style={{ fontWeight: 600 }}>
                        {e.consumed > 0 ? '-' : e.consumed < 0 ? '+' : ''}{Math.abs(e.consumed)} {getUnitLabel(e.unit, true)}
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr className="bg-[#f5f3ef] border-t border-[rgba(0,0,0,0.08)]">
                    <td colSpan={4} className="px-4 py-2.5 text-sm text-right text-[#717182]">Total consumido:</td>
                    <td className="px-4 py-2.5 text-sm text-right text-orange-700" style={{ fontWeight: 700 }}>
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-3 text-left">
                  <div
                    className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      hasWarehouseChanges ? 'bg-amber-100' : isExpanded ? 'bg-[#3d7a3d]/10' : 'bg-muted'
                    }`}
                  >
                  ...
                  {hasWarehouseChanges && (
                    <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full" style={{ fontWeight: 500 }}>Modificado</span>
                  )}
                </div>
                {isExpanded ? <ChevronUp size={18} className="text-[#717182] flex-shrink-0" /> : <ChevronDown size={18} className="text-[#717182] flex-shrink-0" />}
              </button>

              {isExpanded && (
                <div className="border-t border-[rgba(0,0,0,0.06)]">
                  {warehouseProducts.length === 0 ? (
                    <p className="text-sm text-[#717182] text-center py-6">Sin productos en este almacén</p>
                  ) : (
                        ...
                        return (
                          <div key={product.id} className="px-5 py-3 flex items-center gap-3">
                            <div className="flex-1 min-w-0">
                              <p className="text-sm" style={{ fontWeight: 500 }}>{product.name}</p>
                              <p className="text-xs text-[#717182]">
                                Stock previo: {edit.previousStock} {getUnitLabel(product.unit, true)}
                                {consumed !== 0 && (
                                  <span className={`ml-2 ${consumed > 0 ? 'text-orange-600' : 'text-blue-600'}`} style={{ fontWeight: 600 }}>
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              ...
                              <span className="text-xs text-[#717182] w-6">{getUnitLabel(product.unit, true)}</span>
                            </div>
                          </div>
                        );
                      })}
      ...
      <div className="sticky bottom-4 flex justify-end">
        <button
          onClick={handleSave}
          disabled={!hasChanges}
          className={`flex items-center gap-2 px-6 py-3 rounded-xl text-white text-sm shadow-lg transition-all ${hasChanges ? 'bg-[#3d7a3d] hover:bg-[#2f5f2f] active:scale-95' : 'bg-gray-300 cursor-not-allowed'}`}
        >
          <Check size={18} />
