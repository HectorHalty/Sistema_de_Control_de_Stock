...
import { canAccessModule, getAllowedModules, getRoleLabel, shouldShowBottomNavigation, type ModuleId } from './platformAccess';
...
function buildContextNavItems(pathname: string, search: string, activeModule: ModuleId): ContextNavItem[] {
  ...
  if (activeModule === 'online') {
    ...
    return [
      ...
      { key: 'integracion', label: 'Integración', icon: Globe },
    ].map(item => ({
      label: item.label,
...
export function AppLayout({ onLogout }: AppLayoutProps) {
  ...
  const sidebarModules = useMemo(() => {
    ...
    return ['dashboard', ...allowed] as ModuleId[];
  }, [currentUser.role]);

  const bottomButtons: ModuleId[] = ['ventas', 'online', 'dashboard', 'futbol', 'stock'];
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          {contextNavItems.length > 0 ? (
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-4 rounded-2xl px-4 py-3 text-left text-[1.05rem] transition-colors ${active ? 'bg-[#3d7a3d] text-white' : 'text-foreground hover:bg-muted'} ${blocked ? 'opacity-50' : ''}`}
              >
                <Icon size={24} />
                {!collapsed && <span>{item.label}</span>}
              </button>
      ...
      <div className={`flex min-h-screen flex-col transition-all ${collapsed ? 'lg:pl-[88px]' : 'lg:pl-72'}`}>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <img src={logoIcon} alt="Usuario" className="logo-sidebar h-9 w-9 rounded-full border-2 border-[#3d7a3d] object-cover" />
          </div>
        </header>
      ...
      {showBottomNavigation && (
        <nav className={`fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.35rem)] pt-2 backdrop-blur lg:inset-x-auto lg:bottom-4 lg:right-4 lg:rounded-2xl lg:border lg:shadow-xl ${collapsed ? 'lg:left-[104px]' : 'lg:left-[304px]'}`}>
          <div className="mx-auto grid max-w-xl grid-cols-5 items-end gap-1 lg:max-w-3xl">
            {bottomButtons.map(moduleId => {
              const item = moduleMeta[moduleId];
              const Icon = item.icon;
              const active = activeModule === moduleId;
              const blocked = moduleId !== 'dashboard' && !canAccessModule(currentUser.role, moduleId);
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center rounded-xl px-1 py-1.5 text-[11px] transition-all ${
                    isCenter
                      ? `-mt-7 h-16 w-16 justify-center self-start justify-self-center rounded-full border-4 border-background ${active ? 'bg-[#3d7a3d] text-white' : 'bg-[#3d7a3d] text-white'}`
                      : `h-12 ${active ? 'text-[#3d7a3d]' : 'text-muted-foreground'} ${blocked ? 'opacity-40' : ''}`
                  }`}
                >
