...
export function ReturnsModule() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pb-20 lg:pb-4">
      <div className="bg-card rounded-xl border border-border p-4 flex flex-col min-h-[60vh]">
        <div className="flex items-center gap-2 mb-3">
          <RotateCcw className="w-5 h-5 text-amber-600 dark:text-amber-400" />
          <h3 className="text-foreground">Productos vendidos</h3>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5">
          {returnableProducts.length === 0 ? (
            <div className="text-center text-muted-foreground py-12 text-sm px-2">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left rounded-lg p-2 flex items-center gap-2 transition ${
                    canAdd
                      ? "bg-muted hover:bg-amber-50 dark:hover:bg-amber-950/40"
                ...
                >
                  ...
                  {canAdd && <Plus className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0" />}
                </button>
              );
            })
          )}
      ...
      <div className="bg-amber-50/40 dark:bg-amber-950/40 rounded-xl border border-amber-200 dark:border-amber-900 p-4 flex flex-col min-h-[60vh]">
        <h3 className="text-foreground mb-1">Productos a devolver</h3>
          ...
          {items.length === 0 ? (
            ...
            items.map((i) => {
              ...
              return (
                <div
                  key={i.productId}
                  className="bg-card rounded-lg p-2 flex items-center gap-2"
                >
                  ...
                  <button
                    ...
                    onClick={() => updateQty(i.productId, 1)}
                    className="w-7 h-7 rounded bg-muted flex items-center justify-center disabled:opacity-40"
                  >
        ...
        <div className="space-y-2 border-t border-amber-200 dark:border-amber-900 pt-2">
          <div className="flex justify-between items-center">
            <span className="text-foreground text-sm">Total a devolver</span>
      ...
      {preview && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-muted rounded-xl p-4 max-h-[90vh] overflow-auto">
            <div className="flex justify-between items-center mb-3">
              <span className="text-foreground text-sm">Comprobante de devolución</span>
