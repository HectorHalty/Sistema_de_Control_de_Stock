...
export function SalesExportReport() {
  ...
  const report = useMemo(() => {
    ...
    const reports: StationReport[] = Array.from(stationData.keys()).map((station) => {
      ...
      const totalRevenue = stationProducts.reduce((sum, p) => sum + p.total, 0);
      const totalUnits = stationProducts.reduce((sum, p) => sum + p.quantity, 0);

      return { station, products: stationProducts, totalRevenue, totalUnits };
    });
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h3 className="text-2xl text-gray-900 dark:text-gray-100">Ventas por cocina</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Resumen por estación y exportación a Excel
          </p>
        </div>

        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex gap-2 items-center">
            <label className="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Desde:</label>
            <input
              type="date"
              value={dateFrom}
          ...
          <div className="flex gap-2 items-center">
            <label className="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Hasta:</label>
            <input
              type="date"
              value={dateTo}
        ...
        {report.map((stationReport) => (
          ...
          >
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div>
                <h4 className="text-lg text-gray-900 dark:text-gray-100">{stationReport.station}</h4>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {stationReport.products.length} productos • {stationReport.totalUnits} unidades •{" "}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                        <th className="px-4 py-3 text-left text-sm text-gray-700 dark:text-gray-300">Producto</th>
                        <th className="px-4 py-3 text-right text-sm text-gray-700 dark:text-gray-300">Cantidad</th>
                        <th className="px-4 py-3 text-right text-sm text-gray-700 dark:text-gray-300">Precio Unit.</th>
                        <th className="px-4 py-3 text-right text-sm text-gray-700 dark:text-gray-300">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {stationReport.products.map((product) => (
                        <tr
                          key={product.name}
                          className="border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30"
                        >
                          <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{product.name}</td>
                          <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{product.quantity}</td>
