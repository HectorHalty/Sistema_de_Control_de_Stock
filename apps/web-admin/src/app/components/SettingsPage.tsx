...
import { Plus, Edit, Trash2, X, Shield, User, Warehouse, CircleDollarSign } from 'lucide-react';
import { useAppContext } from './AppContext';
import logoIcon from '../../assets/logo-LCH.png';
import logo10 from '../../assets/logo-10A.png';

import type { AppUser } from './store';
import { canAccessModule, getRoleLabel } from './platformAccess';
...
export function SettingsPage() {
  ...
  const [tab, setTab] = useState<'users' | 'app' | 'stock' | 'ventas'>('users');
  ...
  const tabs = [
    { key: 'users' as const, label: 'Usuarios', icon: User },
    { key: 'app' as const, label: 'Aplicacion', icon: Shield },
    ...
    <div className="space-y-6">
            ...
            {usersSorted.map(user => (
              <div key={user.id} className="bg-card rounded-xl border border-border shadow-sm p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className={`text-xs px-2.5 py-1 rounded-full flex-shrink-0 ${user.role === 'Admin' || user.role === 'SuperAdmin' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' :
                      user.role === 'Operador' || user.role === 'Gerente_Operaciones' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' :
                ...
                <tbody>
                  {usersSorted.map(user => (
                    <tr key={user.id} className="border-b border-border hover:bg-background">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2.5 py-1 rounded-full ${user.role === 'Admin' || user.role === 'SuperAdmin' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' :
                            user.role === 'Operador' || user.role === 'Gerente_Operaciones' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' :
                              'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'
                          }`} style={{ fontWeight: 500 }}>
                          {user.role}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between py-3 border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Recibir alertas cuando el stock baje del minimo</p>
              </div>
            ...
            <div className="flex items-center justify-between py-3 border-b border-[rgba(0,0,0,0.04)]">
              <div>
                <p className="text-sm" style={{ fontWeight: 500 }}>Dia de Alerta de Stock Faltante</p>
                <p className="text-xs text-muted-foreground">Elegi que dia de la semana queres que te avise si falta stock</p>
              </div>
              <select
                value={stockAlertDay}
            ...
            <div className="flex items-center justify-between py-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Cambiar el tema de la aplicacion</p>
              </div>
              <button
      ...
      {tab === 'stock' && canSeeStock && (
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-4">
          <h3 className="text-foreground">Configuracion de Stock</h3>
          <p className="text-sm text-muted-foreground">Ajustes especificos del modulo de Stock.</p>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-3 border-b border-border">
              <div>
                <p className="text-sm" style={{ fontWeight: 500 }}>Alertas automaticas</p>
                <p className="text-xs text-muted-foreground">Notificar cuando un producto baja de 20 unidades</p>
              </div>
              <div className="w-11 h-6 bg-[#3d7a3d] rounded-full relative cursor-pointer">
                <div className="absolute right-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow" />
              </div>
            ...
            <div className="flex items-center justify-between py-3">
              <div>
                <p className="text-sm" style={{ fontWeight: 500 }}>Unidad de pedido por defecto</p>
                <p className="text-xs text-muted-foreground">Redondeo automatico a multiplos del pack</p>
              </div>
